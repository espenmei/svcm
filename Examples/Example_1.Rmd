...
The **svcmr** interface only requires that the structural equation model is specied to apply for each individual. The user must construct relationship matrices that relates to different variance components, but other than that the required arrangement of data/paremters is handled internally. The above model can be specified in **svcmr** as follows.
```{r specify_model}
library(svcmr)
